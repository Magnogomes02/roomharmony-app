...
function buildRenderData(contract: ContractPdfData, branding: ClinicBranding): ContractTemplateRenderData {
  ...
  return {
    ...
    DATA_INICIO: formatDateOnlyBR(contract.start_date),
    DATA_TERMINO: contract.end_date ? formatDateOnlyBR(contract.end_date) : "prazo indeterminado",
    VALOR_MENSAL: fmtBRL(contract.monthly_value),
    DIA_VENCIMENTO: contract.due_day != null ? String(contract.due_day) : "",
    DATA_ASSINATURA: contract.signed_at ? formatAnyDateBR(contract.signed_at) : formatAnyDateBR(new Date().toISOString()),
    DATA_ATUAL: formatAnyDateBR(new Date().toISOString()),
